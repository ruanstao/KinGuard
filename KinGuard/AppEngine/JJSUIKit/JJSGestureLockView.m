...
#import "JJSGestureLockView.h"

const static NSUInteger kNumberOfNodes = 9;
const static NSUInteger kNodesPerRow = 3;
const static CGFloat kDefaultWidthForNode = 60;
const static CGFloat kDefaultHeightForNode = 60;
const static CGFloat kDefaultWidthForLine = 16;
const static CGFloat kTrackedLocationInvalidInContentView = -1.0;

@interface JJSGestureLockView (){
...
@implementation JJSGestureLockView
...
- (void)lockViewInitialize
{
    ...
    self.buttonSize = CGSizeMake(kDefaultWidthForNode, kDefaultHeightForNode);
    ...
    self.selectedButtons = [@[] mutableCopy];
...
- (void)layoutSubviews
{
    ...
    CGFloat verticalNodeMargin = (self.buttonSize.height * numberOfRows) / (numberOfRows - 1);
    
    float margin = 85;
    if (mIsiP5)
        margin = 60;
    else if (mIsiP6 || mIsiP6Plus)
        margin = 30;
    
    verticalNodeMargin -= margin;
    
    [self.buttons enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        
        int row = (int)idx / self.gestureNodesPerRow;
        int column = (int)idx % self.gestureNodesPerRow;
        UIButton *button = obj;
        [button setFrame:(CGRect){floorf((self.buttonSize.width + horizontalnodeMargin) * column), floorf((self.buttonSize.height + verticalNodeMargin) * row), self.buttonSize.width, self.buttonSize.height}];
...
- (void)drawRect:(CGRect)rect {
    ...
    if ([self.selectedButtons count] > 0) {
        ...
        if (self.trackedLocationInContentView.x != kTrackedLocationInvalidInContentView && self.trackedLocationInContentView.y != kTrackedLocationInvalidInContentView) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (CGRectContainsPoint(self.contentView.bounds, locationInContentView)) {
        ...
        UIButton *touchedButton = [self buttonContainsThePoint:locationInContentView];
        if (touchedButton && [self.selectedButtons indexOfObject:touchedButton] == NSNotFound) {
            
            [touchedButton setSelected:YES];
            [self.selectedButtons addObject:touchedButton];
            if ([self.selectedButtons count] == 1) {
                
                // The touched button is the first button in the selected buttons
...
- (void)setDelegate:(id<JJSGestureLockViewDelegate>)delegate{
    _delegate = delegate;
